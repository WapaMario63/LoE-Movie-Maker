...
void Widget::udpProcessPendingDatagrams()
{
    while (udpSocket->hasPendingDatagrams())
    {
                ...
                && (unsigned char)datagram[2]==0 && datagram.size()>=22)
        {
            ...
            if (is_sesskey_valid)
            {
                ...
                if (newPlayer->IP != rAddr.toString()) // IP:Port not found in player list
                {
                    ...
                    if (n>=maxConnected)
                    {
                        sendMessage(newPlayer, MsgDisconnect, "Too much players connected. Try again later.");
                    }
                    else
                ...
                else  // IP:Port found in player list
                {
                    ...
                    {
                        sendMessage(newPlayer, MsgDisconnect, "Player already connected.");
                        return;
                    }
                    ...
                    if (n>=maxConnected)
                    {
                        sendMessage(newPlayer, MsgDisconnect, "Server Full. Try again later.");
                    }
            ...
            else
            {
                ...
                logMessage("[INFO] UDP: Sesskey rejected: got '"+badHash+"' instead of '"+sesskey.left(32)
                           +"', passhash was '"+QString(sesskey.right(40)));
                Player* newPlayer = new Player;
                newPlayer->IP = rAddr.toString();
                newPlayer->port = rPort;
                sendMessage(newPlayer, MsgDisconnect, "Kicked from server. \nReason: Wrong Server Password \n\nThis server is protected with a salt password.");
                return;
            }
        }
        ...
        {
            logMessage("[INFO] UDP: Request from unknown peer rejected : "+rAddr.toString()+":"+QString().setNum(rPort));
